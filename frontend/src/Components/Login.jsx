    ...
    <>
      ...
      <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-sm">
          ...
          {isLoginForm? "Login " : "Signup"  } Here</h2>
        </div>
        <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <InputField value={email} setValue ={setEmail} label="Email address" type="email" />
            ...
            {response && <Response message={response?.message} status={response?.status} />}
          </form>
          <p className="mt-10 text-center text-sm/6 text-gray-400">
            {isLoginForm? "Not a member? " : "Already a member? "}{' '} 
            <button onClick={()=>setIsLoginForm(!isLoginForm)} className="font-semibold text-indigo-400 hover:text-indigo-300 cursor-pointer">
              {isLoginForm? "Register now" : "Login now"}</button>
          </p>
        </div>
      </div>
    </>
